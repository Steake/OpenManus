"""
Unit tests for MarketplaceArbitrageTool.
...
async def test_find_opportunities_success(mock_tool_with_db):
    ...
    # Mock the scraping and search methods
    mock_products = [
...
async def test_find_opportunities_database_storage(mock_tool_with_db):
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            with engine.connect() as c:
                count = c.execute(
                    text("SELECT COUNT(*) FROM arbitrage_products")
                ).fetchone()[0]
                assert count > 0
...
@pytest.mark.asyncio
async def test_execute_find_opportunities(mock_tool_with_db):
    """Test action routing for find_opportunities."""
    tool = mock_tool_with_db
...
@pytest.mark.asyncio
async def test_end_to_end_workflow(mock_tool_with_db):
    """Test complete workflow: find -> create -> sync -> monitor."""
    tool = mock_tool_with_db

    # Step 1: Find opportunities (mocked)
    mock_products = [create_test_product()]
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            find_result = await tool.find_opportunities(
                source_platforms=["amazon"],
                target_platforms=["ebay"],
                min_profit_margin=0.20,
            )

            output = json.loads(find_result.output)
            assert output["total_opportunities"] > 0
            ...
            with engine.connect() as c:
                product_id = c.execute(
                    text("SELECT id FROM arbitrage_products ORDER BY id DESC LIMIT 1")
                ).fetchone()[0]
            ...
            with patch.object(
                ...
                mock_gen.return_value = {"title": "Test", "description": "Test"}

                listing_result = await tool.create_listing(
                    product_id=product_id, target_platform="ebay"
                )

                listing_output = json.loads(listing_result.output)
                listing_id = listing_output["listing_id"]

            # Step 3: Sync inventory
            sync_result = await tool.sync_inventory(listing_ids=[listing_id])
            sync_output = json.loads(sync_result.output)
            assert sync_output["total_synced"] == 1

            # Step 4: Monitor competition
            monitor_result = await tool.monitor_competition(product_id=product_id)
            monitor_output = json.loads(monitor_result.output)
            assert "competition_analysis" in monitor_output
